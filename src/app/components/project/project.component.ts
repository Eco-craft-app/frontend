...
export class ProjectComponent {
  ...
  projectRedirect(e: FocusEvent) {
    ...
    if (
      target.classList[0].includes('likes') ||
      target.classList[0].includes('delete') ||
      target.classList[0].includes('comment')
    ) {
      return;
    }
    this.router.navigate([
      `/recycle/project/${this.projectData().projectId}/description`,
    ]);
  }
  ...
  userProfileRedirect() {
    ...
    this.router.navigate([
      `/recycle/profile/${this.projectData().userId}/projects`,
    ]);
  }

  deleteProject() {
    const filter = this.isUserProfile()
      ? { userId: this.projectData().userId! }
      : undefined;
    ...
    const sub = this.projectService
      ...
      .subscribe({
        next: (res) => {
          this.projectService
            .getProjects(
              this.projectService.page(),
              this.projectService.pageSize(),
              filter,
              this.projectService.sortOrder() === 'asc'
                ? 'likeCount'
                : '-likeCount'
            )
            .subscribe({
  ...
  toggleLike() {
    ...
    const sub = this.projectService
      .toggleLike(
        ...
        this.projectData().isLikedByCurrentUser
      )
      .subscribe({
        next: (res) => {
          this.projectService
